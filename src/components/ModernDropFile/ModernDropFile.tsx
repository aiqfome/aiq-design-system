...
const Container = styled(Flex)<ContainerProps>`
  border: 1px solid
    ${({ error, theme }) => (error ? theme.colors.red : theme.colors.primary)};
...
export const ModernDropFile = React.forwardRef<HTMLInputElement, Props>(
  ({
    ...
    return (
      <Flex flexDirection='column' width='360px' {...props}>
        <Container
        ...
        >
          ...
          />
          <Flex gap='8px'>
            <Icon color='primary'>{suffixIcon || <MdFileUpload />}</Icon>
            <Text color='primary'>{translate.message.toUpperCase()}</Text>
          </Flex>
        </Container>
        <Text
          mt={4}
...
ModernDropFile.displayName = 'ModernDropFile'
